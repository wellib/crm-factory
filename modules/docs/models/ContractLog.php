...
/**
 * This is the model class for collection "docs_contract_log".
 *
 ...
 */
class ContractLog extends \yii\mongodb\ActiveRecord
{

    const NOTIFY_contract_CREATED                      = 'notify_contract_created'; // задача создана
    const NOTIFY_contract_UPDATED                      = 'notify_contract_updated'; // задача обновлена + данный которые обновились
    
    const NOTIFY_contract_AWAITING_APPROVAL            = 'notify_contract_awaiting_approval'; // ожидает согласования
    const NOTIFY_USER_APPROVED_Contract                = 'notify_user_approved_Contract'; // пользователь ОДОБРИЛ выполнение задачи(утвердил/согласовал)
    const NOTIFY_USER_DISAPPROVED_Contract             = 'notify_user_disapproved_Contract'; // пользователь НЕ ОДОБРИЛ выполнение задачи(не утвердил/ несогласовал)
    const NOTIFY_contract_DENIED_APPROVAL              = 'notify_contract_denied_approval'; // Задача не прошла согласование
    const NOTIFY_AGAIN_APPROVE                     = 'notify_again_approve'; // задача отправлена на повторное согласование
    const NOTIFY_contract_APPROVED                     = 'notify_contract_approved'; // задача утверждена(прошла согласование)
    
    const NOTIFY_contract_AWAITING_EXECUTION           = 'notify_contract_awaiting_execution'; // Ожидает начала работы
    ...
    {
        return [
            [['contract_id', 'type', '_data', '_user', 'created_at', 'updated_at', 'comment'], 'safe'],
    ...
    {
        return [
            '_id' => Module::t('contract_log', 'ID'),
            'contract_id' => Module::t('contract_log', 'Contract ID'),
            'type' => Module::t('contract_log', 'Type'),
            '_data' => Module::t('contract_log', 'Data'),
            'comment' => $this->getScenario() == self::SCENARIO_DEFAULT ? Module::t('contract_log', 'ATTR__REASON__LABEL') : Module::t('contract_log', 'Комментарий'),
            'attachedFilesUpload' => Module::t('contract_log', 'Добавить файлы'),
            'created_at' => Module::t('contract_log', 'Created At'),
            'updated_at' => Module::t('contract_log', 'Updated At'),
        ];
    }
    ...
    {
        return $this->hasOne(Contract::className(), ['_id' => 'contract_id']);
    }
     ...
     * @return bool
     */
    public static function createNotify($Contract, $type, $user_id = null, $data = [])
    {
        if (($Contract instanceof Contract) === false) {
            return false;
        }
        $model = new self();
        $model->contract_id = $Contract->_id;
        $model->type = $type;
        $model->_data = $data;
        $model->_user = $user_id;
    ...
    {
        return Url::to(['/docs/Contract/download-attached-file', 'id' => (string) $this->_id, 'filename' => $filename, 'type' => 'log'], $scheme);
    }
    ...
    public function getDiffContractModelDataAfterUpdate()
    {
        ...
        foreach ($this->_data['oldModelData'] as $attributeName => $attributeValue) {
            if ($attributeValue != $this->_data['newModelData'][$attributeName]) {
                switch ($attributeName) {
                    case 'priority':
                        $diff[] = [
                            'label' => $this->Contract->getAttributeLabel($attributeName),
                            'oldValue' => Contract::priorityLabel($this->_data['oldModelData'][$attributeName]),
                            'newValue' => Contract::priorityLabel($this->_data['newModelData'][$attributeName]),
                        ];
                        break;
