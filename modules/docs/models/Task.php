...
/**
 * This is the model class for collection "docs_task".
 *
 * @property \MongoDB\BSON\ObjectID|string $_id
 * @property \MongoDB\BSON\ObjectID|string|null $_parent
 * @property boolean $template Является ли служебка шаблонной, т.е. будут ли на её основе создаваться клоны служебк - используется в служебках которые выполняются циклически (каждый день, кажую неделю, каждый месяц)
 *
 ...
 *
 *
 * @property string $approve_execute_deadline_timestamp Крайник срок до которого нужно утвердить(начало выполнения) иначе служебка будет считаться просроченной на этапе утверждения
 * @property string $check_results_deadline_timestamp  Крайник срок до которого нужно проверить результат выполненой служебки, иначе служебк будет считаться просроченной на этапе проверки результата
 *
 ...
 *
 * @property \MongoDB\BSON\ObjectID|string $_author Пользователь создавший служебку
 * @property User $author
 *
 *
 * @property array $_users_approve_execute         Пользователи которые должны одобрить данную служебку к выполнению, т.е. разрешить выполнение
 * @property User[] $usersApproveExecute
 * @property array $_users_approve_execute_response Пользователи которые дали свои ответы(разрешили или запретили выполнение служебки)
 *
 * @property array $_users_performers          Исполнители - пользователи которые выполняют(будут выполнять) данную служебку
 * @property User[] $usersPerformers
 * @property array $_users_performers_finished Исполнители которые завершили выполнение служебки
 *
 *
 * @property array $_users_check_result         Пользователи которые осуществляют проверку результата выполненной служебки, они решают выполнена служебка или нет
 * @property User[] $usersCheckResult
 * @property array $_users_check_result_response Пользователи которые проверили результат выполненной служебки и дали свой ответ (положительный - приняли результат и подвердили что служебка выполнена или отрицательный - не приняли результат и считают что служебка не выполнена)
 *
 * @property array $_users_notify_after_finished Пользователи которые должны получить уведомление об успешном выполнении служебки после её завершения
...
class Task extends \yii\mongodb\ActiveRecord
{
    // Приоритеты
    /**
     * Приоритет служебки - низкий
     */
    const PRIORITY__LOW    = 1;
    /**
     * Приоритет служебки - средний
     */
    const PRIORITY__MEDIUM = 2;
    /**
     * Приоритет служебки - высокий
     */
    ...
    /**
     * Не прошла проверку результатов (служебка не выполнена)
     */
    ...
    public function setDeadlines()
    {
        ...
        // установим timestamp с которого нужно начать выполнение данной служебки
        if (!empty($this->start_date)) {
            $this->start_timestamp = strtotime($this->start_date);
        }

        // установим timestamp до какого момента должны создаваться клоны служебк
        // это для циклично выполняющихся служебка по определенному периоду - каждый день / каждую неделю / каждый месяц
        ...
        switch ($this->deadline_type) {
            case self::DEADLINE_TYPE__ONE_TIME:
                // установим timestamp дедлайны служебки которая выполняется 1 раз
                ...
                $this->template = false; // данная служебка выполниться только 1 раз, соответственно не может быть шаблоном для клонирования похожих служебк
                break;
            case self::DEADLINE_TYPE__EVERY_DAY:
            case self::DEADLINE_TYPE__EVERY_WEEK:
                // если служебка выполняется каждый день, то просто помечаем всю неделю [1,2,3,4,5]
                ...
                if (empty($this->deadline_timestamp)) {
                    // если timestamp дедлайна ранее не был установлен (например это новая служебка)
                    // то начнем поиск с даты начала выполнения служебки + время дедлайна которые установил пользователь
                    ...
                    // если ранее уже был установлен дедлайн служебки (например она уже выполняется или выполнялась ранее)
                    // то начнем поиск следующего дедлайна начиная с текущего(последнего) установленного дедлайна
                    $startAt = $this->deadline_timestamp;
                }

                // Если дедлайн ранее не был установлен (т.е. новая служебка), то будем так же учитывать "сегодня"
                // т.е. что бы уже сейчас/сегодня начать выполнение служебки, если вдруг например ближайший дедлайн выпадет на сегодня
                $today = empty($this->deadline_timestamp) ? true : false;

                // определеяем и устанавливаем дедлайн служебки
                ...
                $this->template = true; // данная служебка выполниться несколько раз, является шаблон для клонирования новых служебк
                break;
            case self::DEADLINE_TYPE__EVERY_MONTH:
                // если служебка выполняется в определенные даты каждого месяца
                // определение timestamp(даты) с которой будем производить поиск timestamp для дедлайна
                if (empty($this->deadline_timestamp)) {
                    // если timestamp дедлайна ранее не был установлен (например это новая служебка)
                    // то начнем поиск с даты начала выполнения служебки + время дедлайна которые установил пользователь
            ...
            case self::DEADLINE_TYPE__EVERY_DATE:
                // если служебка выполняется в определенные даты
                ...
                if (empty($this->deadline_timestamp)) {
                    ...
                    $newTimestamp = null;
                    // если ранее уже был установлен timestamp (служебка уже выполнялась как минимум 1 раз)
    ...
    public function createNextTask()
    {
        ...
        // если сегодняшняя дата = дате после которой служебка не будет клонироваться
    ...
    {
        $this->template = false; // текущая модель больше не является шаблоном который участвует в выборке для создания следующей аналогичной модели(служебки)
        ...
        //$clone->_users_approve_execute_response = []; // Очищачем список тех кто дал разрешение на выполнение, что бы клон служебки ушел на согласование согласование
        //$clone->_users_check_result_response = []; // Очищачем список тех кто проверял старую служебку, что бы в клоне служебке все было чисто (с нуля)
    ...
    {
        return ['/docs/task/user-action', 'id' => (string) $this->_id, 'type' => $actionType];
    }
        ...
        $actions = [
            ...
            'approve_execute' => [
                ...
                'available' => call_user_func(function() use ($that, $me) {
                    // данная служебка ждет согласования?
                ...
                'action' => function() use ($that, $me) {
                    ...
                    // флешка для того что бы в actionView показать что типа служебка выполнена
                    ...
                    if (count($that->_users_approve_execute) === count($that->_users_approve_execute_response)) {
                        ...
                        // если все подтвердили выполнение служебки
                        if ($approved === true) {
                        ...
                        // если хотя бы 1 не дал свое согласие не выполнение служебки
                        if ($approved === false) {
                            // установим статус "служебка не прошла согласование"
            ...
            'reject_execute' => [
              ...
              //ищем все дочерние служебки для акцепта
            ...
            'again_approve_execute' => [
                'label' => 'Отправить служебку на повторную акцептацию',
                'available' => call_user_func(function() use ($that, $me) {
                    // данная служебка ждет согласования?
                    $check = $that->status === self::STATUS__APPROVAL_FAILED;
                    // Я являюсь автором данной служебки?
            ...
            //'started_perform' => [
            //    'label' => 'Приступить к выполнению служебки',
            //    'available' => call_user_func(function() use ($that, $me) {
            //        // данная служебка ждет начала выполнения
            ...
            //            }
            //            // если все подтвердили начало выполнения служебки
            //            if ($approved === true) {
            ...
            'back_perform' => [
                'label' => 'Продолжить выполнение служебки',
                'available' => call_user_func(function() use ($that, $me) {
                    // служебка сейчас на выполнении или не прошла проверку результата
                    $check = $that->status === self::STATUS__IN_PROGRESS;
                    // если есть контролирующие выполнение юзеры, то будем искать среди них, если нет то среди исполнителей
                    $check = $check && in_array($me, $that->_users_performers);
                ...
                'action' => function() use ($that, $me) {
                    ...
                    //    // сделаем служебку снова выполняемм
                    //    if ($that->status !== self::STATUS_IN_PROGRESS) {
                    //        TaskLog::createNotify($this, TaskLog::NOTIFY_SET_STATUS_IN_PROGRESS);
                    //    }
                    //    $that->status = self::STATUS_IN_PROGRESS;
                    //    // очистим список юзеров которые до этого проверяли результат,
                    //    // для того что бы отправить служебку на повторную проверку результата,
                    //    // после того как юзеры опять подтвердят что служебка готова и её можно проверят
            ...
            'approve_results' => [
                ...
                'available' => call_user_func(function() use ($that, $me) {
                    // данная служебка ждет подтверждения(принятия) STATUS_AWAITING_CHECK_RESULTS
                ...
                'action' => function() use ($that, $me) {
                    ...
                    if (count($that->_users_check_result) === count($that->_users_check_result_response)) {
                        ...
                        if ($approved === true) {
                            ...
                            // т.к. служебка не прошла контроль результата
                            TaskLog::createNotify($this, TaskLog::NOTIFY_SET_STATUS_IN_PROGRESS);
    ...
    /**
     * служебки которые ожидают выполнения(ответа) от пользователя
     *
     * В данном списке отображаются служебки
     * - служебки в которых я являюсь исполнителем и которые имею статус self::STATUS__IN_PROGRESS
     * - qwe
     *
     * @return int|null
     */
    ...
    {
        return Url::to(['/docs/task/download-attached-file', 'id' => (string) $this->_id, 'filename' => $filename], $scheme);
    }
    ...
    /**
     * Проверяет наличие доступа к служебке для ппросмотра, или совершения других двействий
     *
    ...
    {
        return ['/docs/task/view', 'id' => $this->getId()];
    }
